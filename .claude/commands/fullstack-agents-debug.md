...
Parse arguments: `/debug [type]`

## Execution Flow

1. **Parse arguments**: Extract `type` from `$ARGUMENTS`
2. **Load appropriate skill**:
   - `error` → `/skill batch-error-resolution`
   - `logs` → No specific skill needed
   - `performance` → No specific skill needed
   - `api` → `/skill fetch-architecture` (for API understanding)
3. **Execute debug** with the loaded skill context

## Types & Skills

| Type | Skill to Load | Description |
|------|---------------|-------------|
| `error` | `batch-error-resolution` | Diagnose errors and stack traces |
| `logs` | None | Analyze application logs |
| `performance` | None | Profile performance bottlenecks |
| `api` | `fetch-architecture` | Debug API requests/responses |
