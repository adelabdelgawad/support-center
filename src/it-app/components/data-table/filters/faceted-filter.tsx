...
function Faceted<Multiple extends boolean = false>(
  ...
  // Use a ref to always read the latest value in onItemSelect,
  // preventing stale closures when multiple selections happen rapidly
  ...
  return (
    ...
    >
      ...
      {values.length > max ? (
        <Badge
          variant="secondary"
          className={cn("rounded-sm px-1.5 font-normal", badgeClassName)}
        >
...
function FacetedContent(props: React.ComponentProps<typeof PopoverContent>) {
  ...
  return (
    <PopoverContent
      ...
      align="start"
      className={cn(
        "w-[260px] origin-(--radix-popover-content-transform-origin) p-0",
        className,
      )}
    >
