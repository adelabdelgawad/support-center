...
import { Building2, MapPin, Network, FileText, Loader2, MessageSquare, Clock } from 'lucide-react';
import { createBusinessUnit } from '@/lib/api/business-units';
import { useBusinessUnitsActions } from '../../context/business-units-actions-context';
import { toast } from 'sonner';
import type { BusinessUnitCreate } from '@/types/business-units';
...
export default function AddBusinessUnitSheet({
  ...
  onOpenChange,
  regions,
}: AddBusinessUnitSheetProps) {
  const { addBusinessUnitToCache } = useBusinessUnitsActions();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showConfirmDialog, setShowConfirmDialog] = useState(false);
  const [formData, setFormData] = useState<BusinessUnitCreate>({
    name: '',
    description: null,
    network: null,
    businessUnitRegionId: 0,
    workingHours: null,
    whatsappGroupName: null,
    whatsappGroupId: null,
    whatsappOutshiftIntervalMinutes: 30,
  });
  ...
  const handleConfirmCreate = async () => {
    ...
    try {
      const newUnit = await createBusinessUnit(formData);
      await addBusinessUnitToCache(newUnit);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
        ...
        <SheetHeader className="px-1">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <SheetDescription>Create a new business unit in the system</SheetDescription>
            </div>
          </div>
        </SheetHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-6 px-1">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => setFormData({ ...formData, network: e.target.value || null })}
              disabled={isSubmitting}
          ...
          <div className="space-y-4 border-t pt-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData({ ...formData, whatsappGroupName: e.target.value || null })}
                disabled={isSubmitting}
                className="h-10"
              />
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData({ ...formData, whatsappGroupId: e.target.value || null })}
                disabled={isSubmitting}
                className="h-10 font-mono text-sm"
              />
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData({ ...formData, whatsappOutshiftIntervalMinutes: parseInt(e.target.value) || 30 })}
                disabled={isSubmitting}
                className="h-10"
              />
          ...
          <div className="flex gap-3 justify-end pt-4 border-t">
            ...
            <Button
              type="submit"
              disabled={isSubmitting || !formData.name.trim() || !formData.businessUnitRegionId}
              className="min-w-[100px]"
            >
              {isSubmitting ? (
                <>
      ...
      <ConfirmationDialog
        ...
        description={`Are you sure you want to create the business unit "${formData.name}"?`}
        confirmText="Create"
        cancelText="Cancel"
