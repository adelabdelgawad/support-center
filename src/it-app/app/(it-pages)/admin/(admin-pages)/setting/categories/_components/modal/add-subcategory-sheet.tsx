...
export function AddSubcategorySheet({
  ...
  const handleSave = async (data: CreateSubcategoryFormData) => {
    setIsLoading(true);
    try {
      const subcategoryData: SubcategoryCreateRequest = {
        categoryId,
        name: data.name,
        nameEn: data.nameEn,
        nameAr: data.nameAr,
      ...
      toastError(errorMessage || "Failed to create subcategory");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-2xl">
        <SheetHeader>
          ...
          <SheetDescription>
            Add a new subcategory to further organize service requests.
          </SheetDescription>
        </SheetHeader>
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      {...field}
                      placeholder="e.g., إصلاح الكمبيوتر المحمول"
                      dir="rtl"
                    />
                  </FormControl>
                  <FormMessage />
