...
import { updateEmailConfig } from "@/lib/api/email-config";
...
interface EditEmailConfigSheetProps {
  open: boolean;
...
export function EditEmailConfigSheet({
  open,
  onOpenChange,
  config,
  onSuccess,
}: EditEmailConfigSheetProps) {
  ...
  const handleSave = async (data: FormValues) => {
    ...
    try {
      ...
      const result = await updateEmailConfig(config.id, updateData);
      ...
      toastError(error.message || "Failed to update email configuration");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-[600px] overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            Update SMTP settings for {config.name}
          </SheetDescription>
        </SheetHeader>
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    ...
                    <FormDescription>
                      Use this as the default email configuration
                    </FormDescription>
                  </div>
                  <FormControl>
                    <Switch
