...
interface EditADConfigSheetProps {
  open: boolean;
...
export function EditADConfigSheet({
  open,
  onOpenChange,
  config,
  onSuccess,
  ...
  const computedBaseDn = useMemo(() => {
    if (!domainName) return config.baseDn || "Enter domain name above";

    return domainName
      .split(".")
      .map((part) => `DC=${part}`)
      .join(",");
  }, [domainName, config.baseDn]);
  ...
  const handleSave = async (data: FormValues) => {
    ...
    try {
      ...
      const result = await updateADConfig(config.id, updateData);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-[600px] overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Edit Active Directory Configuration</SheetTitle>
          <SheetDescription>
            Update Active Directory server settings
          </SheetDescription>
        </SheetHeader>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Only fill if changing password. Will be encrypted before storing.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
