...
export function OUManagementDialog() {
  ...
  const handleDiscover = async () => {
    ...
    try {
      ...
      const errorMessage = error?.message || "Failed to discover OUs from Active Directory";

      if (errorMessage.includes("invalid server address") || errorMessage.includes("No active AD configuration")) {
        toastError("Please configure and activate an Active Directory server first. Go to Management → Active Directory to add a configuration.");
      } else {
                ...
                {ous.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center text-muted-foreground py-8">
                      No organizational units configured. Click &ldquo;Discover from AD&rdquo; to find available OUs.
                    </TableCell>
                  </TableRow>
                ) : (
                  ous.map((ou) => {
