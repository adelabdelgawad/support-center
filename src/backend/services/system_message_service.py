...
class SystemMessageService:
    ...
    @staticmethod
    @safe_database_query("get_template", default_return=None)
    @log_database_operation("get system message template", level="debug")
    async def get_template(
        db: AsyncSession, message_type: str
    ...
    async def get_bilingual_message(
        ...
        Example:
            en, ar = await get_bilingual_message(
                db,
                "new_request",
                {"request_title": "Fix login issue", "requester_name": "John"}
            )
        """
        template = await SystemMessageService.get_template(db, message_type)
        ...
        msg_en = SystemMessageService.format_message(template.template_en, placeholders)
        msg_ar = SystemMessageService.format_message(template.template_ar, placeholders)
