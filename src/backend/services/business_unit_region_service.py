...
from models import BusinessUnitRegion
from schemas.business_unit_region import (BusinessUnitRegionCreate,
...
class BusinessUnitRegionService:
    ...
    async def list_business_unit_regions(
        ...
        """
        # Build main query
        stmt = select(BusinessUnitRegion).where(BusinessUnitRegion.is_deleted == False)
        ...
        ).where(BusinessUnitRegion.is_deleted == False)
