...
class SchedulerService:
    ...
    async def list_job_types(
        ...
        result = await db.execute(
            ...
            .where(SchedulerJobType.is_active == True)
    ...
    async def list_scheduled_jobs(
        ...
        # Apply filters
        if name:
            query = query.where(ScheduledJob.name.ilike(f"%{name}%"))
        ...
        enabled_result = await db.execute(
            select(func.count()).where(ScheduledJob.is_enabled == True)
        )
    ...
    async def get_scheduler_status(
        ...
        leader_result = await db.execute(
            ...
            .where(SchedulerInstance.is_leader == True)
        )
        leader = leader_result.scalar_one_or_none()
    ...
    async def timeout_stale_executions(
        ...
        the timeout period and marks them as 'timeout'. This can happen due to:
        ...
        # Get job IDs of timed out executions
        result = await db.execute(
            select(ScheduledJobExecution.job_id)
            .where(
                and_(
                    ScheduledJobExecution.status.in_(["pending", "running"]),
                    ScheduledJobExecution.started_at < timeout_cutoff,
                )
            )
        )
        job_ids = [row[0] for row in result.fetchall()]

        # Update executions to timeout status
        update_result = await db.execute(
            ...
            .values(
                status="timeout",
                completed_at=datetime.utcnow(),
                error_message=f"Execution timed out after {timeout_minutes} minutes (stuck in pending/running)",
            )
        )
        ...
        if job_ids:
            await db.execute(
                ...
                .values(last_status="timeout")
            )
            await db.commit()

        logger.info(
            f"Timed out {count} stale scheduler job execution records "
            f"(timeout: {timeout_minutes} minutes, cutoff: {timeout_cutoff.isoformat()})"
        )
