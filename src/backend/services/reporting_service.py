...
class ReportingService:
    ...
    async def get_executive_dashboard(
        ...
        sla_met_stmt = select(func.count(ServiceRequest.id)).where(
            and_(
                *base_conditions,
                ServiceRequest.sla_resolution_breached == False,
                ServiceRequest.resolved_at != None,
            )
        )
        ...
        avg_frt_stmt = select(
            ...
            and_(
                *base_conditions,
                ServiceRequest.first_response_at != None,
            )
        )
    ...
    async def _get_business_unit_distribution(
        ...
        stmt = (
            ...
            .where(and_(*base_conditions))
            .group_by(BusinessUnit.id, BusinessUnit.name_en)
            .order_by(func.count(ServiceRequest.id).desc())
        )
        result = await db.execute(stmt)
        rows = result.all()
    ...
    async def _get_compliance_by_priority(
        ...
        stmt = (
            select(
                ...
                func.sum(
                    case(
                        (ServiceRequest.sla_resolution_breached == False, 1),
                        else_=0
                    )
                ).label("met"),
            )
    ...
    async def get_sla_compliance_report(
        ...
        frt_met_stmt = select(func.count(ServiceRequest.id)).where(
            and_(
                *base_conditions,
                ServiceRequest.first_response_at != None,
                ServiceRequest.sla_first_response_breached == False,
            )
        )
        ...
        frt_breached_stmt = select(func.count(ServiceRequest.id)).where(
            and_(
                *base_conditions,
                ServiceRequest.sla_first_response_breached == True,
            )
        )
        ...
        res_breached_stmt = select(func.count(ServiceRequest.id)).where(
            and_(
                *base_conditions,
                ServiceRequest.sla_resolution_breached == True,
            )
        )
    ...
    async def get_agent_performance_report(
        ...
        # Get all technicians
        tech_stmt = select(User).where(User.is_technician == True, User.is_active == True)
