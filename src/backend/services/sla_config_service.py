...
from models.database_models import SLAConfig, Priority, Category, BusinessUnit, ServiceRequest
from schemas.sla_config import SLAConfigCreate, SLAConfigUpdate
...
class SLAConfigService:
    ...
    async def list_sla_configs(
        ...
        if active_only:
            stmt = stmt.where(SLAConfig.is_active == True)
        ...
        if category_id:
            configs_to_try.append(
                and_(
                    SLAConfig.priority_id == priority_id,
                    SLAConfig.category_id == category_id,
                    SLAConfig.business_unit_id == None,
                    SLAConfig.is_active == True,
                )
            )
    ...
    def calculate_sla_due_date(
        ...
        hours_per_day = business_end_hour - business_start_hour  # 8 hours
