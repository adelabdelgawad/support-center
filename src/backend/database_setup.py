...
from models import (  # NOTE: AssignType and SessionType are now enums in model_enum.py, not tables; They no longer need to be seeded in the database
    BusinessUnit,
    BusinessUnitRegion,
    Category,
    Page,
...
class DatabaseSetup:
    ...
    def __init__(self):
        ...
        logger.info(f"Database setup initialized with admin config:")
    ...
    async def create_roles(self, db: AsyncSession) -> bool:
        ...
        roles_data = [
            ...
            {
                "name": "auditor",
                "description": "Reviews and audits service quality",
            },
            {
                "name": "manager",
                "description": "Manages Section operations",
            },
            {
                "name": "administrator",
    ...
    async def create_page_role_permissions(
        ...
        try:
            ...
            admin_role_id = role_map["administrator"]

            # Define permissions
            # 1. Requests page (ID: 21) - All roles
            requests_page_id = 21
            all_roles_pages = [requests_page_id]

            # 2. Settings pages - Administrator only
            admin_only_pages = [
                11,  # Roles
                12,  # Users
                13,  # Regions
            ...
            manager_admin_roles = ["manager", "administrator"]
            reports_pages = [
            ...
            # 4. Management pages - All technician roles (technician, senior, supervisor, manager, administrator)
            technician_roles = ["technician", "senior", "supervisor", "manager", "administrator"]
            management_pages = [
                41,  # Active Sessions
                42,  # Deployments
            ]
            ...
            # Assign Settings pages to Administrator only
            for page_id in admin_only_pages:
                ...
                if existing:
                    logger.info(
                        f"Permission already exists: administrator -> Page {page_id}"
                    )
                    continue
                ...
                permissions_created += 1
                logger.info(
                    f"✅ Created permission: administrator -> {page_name} (ID: {page_id})"
                )

            # Assign Reports pages to Manager and Administrator roles
            for role_name in manager_admin_roles:
    ...
    async def setup_default_data(self, db: AsyncSession) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ All lookup tables seeded")
            logger.info(
                f"✅ 2 assignment types created (Assignees and CC only)"
            )
            logger.info(
                f"✅ 6 categories with 12 subcategories created (with bilingual EN/AR names)"
            )
            logger.info(
                f"✅ 28 default tags created across all categories (Network, Software, Hardware, Servers, ERP, Healthcare)"
            )
            logger.info(
                f"✅ 8 default statuses created (Open, Hold, Solved, Archived, Canceled, Pending Sub-Task, Pending Requester Response, In Progress)"
            )
            logger.info(
                f"✅ 8 default statuses with bilingual names (EN/AR) seeded"
            )
            logger.info(f"✅ 5 priorities created")
            logger.info(
            ...
            logger.info(
                f"✅ 3 system message templates created for bilingual notifications"
            )
            logger.info(
                f"✅ Bilingual support fully initialized (Categories, Subcategories, Tags, Statuses)"
            )
