...
class BusinessUnitRegionRepository(BaseRepository[BusinessUnitRegion]):
    ...
    async def find_all_with_counts(
        ...
        stmt = select(BusinessUnitRegion).where(
            BusinessUnitRegion.__table__.c.is_deleted.is_(False)
        )

        total_count_stmt = select(
            func.count().label("total"),
            func.count(case((BusinessUnitRegion.__table__.c.is_active.is_(True), 1))).label(
                "active_count"
            ),
            func.count(case((BusinessUnitRegion.__table__.c.is_active.is_(False), 1))).label(
                "inactive_count"
            ),
        ).where(BusinessUnitRegion.__table__.c.is_deleted.is_(False))
        ...
        if name:
            name_filter = BusinessUnitRegion.__table__.c.name.ilike(f"%{name}%")
            stmt = stmt.where(name_filter)

        if is_active is not None:
            stmt = stmt.where(BusinessUnitRegion.__table__.c.is_active == is_active)

        stmt = stmt.order_by(BusinessUnitRegion.__table__.c.name)
