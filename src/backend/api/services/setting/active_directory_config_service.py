...
from api.repositories.setting.organizational_unit_repository import (
...
class ActiveDirectoryConfigService:
    ...
    async def get_all_configs(
        ...
        configs, total = await ActiveDirectoryConfigRepository.find_paginated(
    ...
    async def get_config_by_id(
        ...
        """
        config = await ActiveDirectoryConfigRepository.find_by_id(db, config_id)
        if not config:
            return None
    ...
    async def create_config(
        ...
        if config_data.is_active:
            await ActiveDirectoryConfigRepository.deactivate_all(db)
        ...
        config = await ActiveDirectoryConfigRepository.create(db, obj_in=config_dict)
    ...
    async def update_config(
        ...
        # Update the config
        updated_config = await ActiveDirectoryConfigRepository.update(
            db, id_value=config_id, obj_in=update_dict
        )
    ...
    async def delete_config(db: AsyncSession, config_id: UUID) -> bool:
        ...
        await ActiveDirectoryConfigRepository.delete(db, id_value=config_id)
