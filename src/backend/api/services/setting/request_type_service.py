...
from sqlalchemy import ColumnElement, select, func, case
...
class RequestTypeService:
    ...
    async def list_request_types(
        ...
        """
        # Base query
        is_active_col = cast(ColumnElement[bool], RequestType.is_active)
        stmt = select(RequestType).order_by(cast(Any, RequestType.id))
        ...
        if is_active is not None:
            stmt = stmt.where(cast(ColumnElement[bool], RequestType.is_active == is_active))
            count_stmt = count_stmt.where(cast(ColumnElement[bool], RequestType.is_active == is_active))

        if name:
            # Search in both English and Arabic names
            name_filter = cast(Any, RequestType.name_en).ilike(f"%{name}%") | cast(Any, RequestType.name_ar).ilike(f"%{name}%")
    ...
    async def get_request_type(
        ...
        """
        stmt = select(RequestType).where(cast(ColumnElement[bool], RequestType.id == request_type_id))
        result = await db.execute(stmt)
    ...
    async def bulk_update_request_type_status(
        ...
        """
        stmt = select(RequestType).where(cast(ColumnElement[bool], cast(Any, RequestType.id).in_(type_ids)))
        result = await db.execute(stmt)
        request_types = list(result.scalars().all())
    ...
    async def is_request_type_in_use(
        ...
        stmt = select(cast(Any, ServiceRequest.id)).where(
            cast(ColumnElement[bool], ServiceRequest.request_type_id == request_type_id)
        ).limit(1)
        result = await db.execute(stmt)
