...
class EmailConfigService:
    ...
    async def get_all_configs(
        ...
        configs, total = await EmailConfigRepository.find_paginated(
    ...
    async def get_config_by_id(
        ...
        """
        config = await EmailConfigRepository.find_by_id(db, config_id)
        if not config:
            return None
    ...
    async def get_active_config(
        ...
        """
        config = await EmailConfigRepository.get_active_config(db)
        if not config:
            return None
    ...
    async def create_config(
        ...
        try:
            ...
            if config_data.is_active:
                await EmailConfigRepository.deactivate_all(db)

            # Create the configuration
            config = await EmailConfigRepository.create(db, obj_in=config_dict)
    ...
    async def update_config(
        ...
        try:
            ...
            updated_config = await EmailConfigRepository.update(
                db, id_value=config.id, obj_in=update_dict
            )
    ...
    async def delete_config(db: AsyncSession, config_id: UUID) -> bool:
        ...
        await EmailConfigRepository.delete(db, id_value=config_id)
        return True
