...
class ChatService:
    ...
    async def create_message(
        ...
        await db.execute(
            ...
            .where(ServiceRequest.id == message_data.request_id)  # type: ignore[arg-type]
            .with_for_update()
        )
        ...
        max_seq_result = await db.execute(
            ...
            .where(ChatMessage.request_id == message_data.request_id)  # type: ignore[arg-type]
        )
    ...
    async def delete_message(db: AsyncSession, message_id: int, user_id: int) -> bool:
        ...
        await ChatMessageRepository.delete(db, id_value=message_id, commit=True)
    ...
    async def _get_chat_messages_list(
        ...
        requests = await ChatMessageRepository.get_requests_with_last_message(
            db, str(user_id), status_filter=status_filter
        )

        # Build chat messages list
        chat_messages = []
        for request in requests:
            # Skip if request id is missing (shouldn't happen, but type safety)
            if request.id is None:
                continue

            # Assert id is not None for type checker (we checked above)
            request_id = request.id
    ...
    async def _get_all_request_statuses(
        ...
        return [
            ChatRequestStatus(
                id=status.id if status.id is not None else 0,
                name=status.name,
                name_en=status.name_en,
                name_ar=status.name_ar,
                color=status.color or "gray",
            )
    ...
    async def _get_request_participants(
        ...
        Args:
            db: Database session
            request_id: Service request ID (int)

        Returns:
            List of user UUIDs (requester + assigned technicians)
        """
        from api.repositories.support.request_repository import ServiceRequestRepository
    ...
    async def _search_chat_messages_list(
        ...
        for request in requests:
            ...
            if read_filter == "read" and unread_for_request > 0:
                ...
                logger.debug(
                    f"[SEARCH_SERVICE] Skipped request {request_id}: has unread messages but filter='read'"
                )
                continue
            if read_filter == "unread" and unread_for_request == 0:
                skipped_by_read_filter += 1
                logger.debug(
                    f"[SEARCH_SERVICE] Skipped request {request_id}: no unread messages but filter='unread'"
                )
                continue
