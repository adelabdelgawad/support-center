...
async def list_requests(
    ...
    **Notes:**
    ...
    # Convert ServiceRequest models to ServiceRequestListItem schemas
    from api.schemas.service_request import ServiceRequestListItem

    list_items = [
        ServiceRequestListItem(
            id=req.id if req.id is not None else 0,  # Should never be None for existing records
            title=req.title,
    ...
    return ServiceRequestList(
        items=list_items, total=total, page=page, per_page=per_page, pages=pages
    )
...
async def update_request(
    ...
    **Notes:**
    ...
    old_status_name_en: str | None = None
    old_status_name_ar: str | None = None
    new_status_name_en: str | None = None
    new_status_name_ar: str | None = None
    ...
    # Trigger system event if status changed (only if we have valid bilingual status names)
    if status_changed and old_status_name_en and new_status_name_en and old_status_name_ar and new_status_name_ar:
        try:
...
async def get_request_assignees(
    ...
    **Returns:**
    ...
    assignees = [
        AssigneeInfo(
            id=a.id if a.id is not None else 0,  # Should never be None for existing records
