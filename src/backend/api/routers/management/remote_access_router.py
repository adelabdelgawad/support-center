...
async def get_request_remote_sessions(
    ...
    **Response Headers:**
    ...
    # Convert database models to schema items
    items = [RemoteAccessSessionRead.model_validate(session) for session in sessions]
...
async def start_remote_access_by_user(
    ...
    **Notes:**
    ...
    # Note: requester_view param filters by visible_on_requester_page=True
    ...
    # Filter to user's requests only
    ...
    if not request:
        ...
        user = await UserRepository.find_by_id(db, str(user_id))
