"""
Client Version API endpoints (Version Authority).

Provides CRUD operations for the client version registry, which manages
version information for all client applications (desktop, web, mobile).

The Version Authority system enables:
- Version checking and enforcement
- Silent/auto-update notifications
- Changelog/release notes distribution
- Installer file management

Key Features:
- Multi-platform support (desktop, web, mobile)
- Version ordering via order_index (higher = newer)
- Latest version flag per platform
- Enforced update flag for mandatory upgrades
- Active/inactive toggle for version lifecycle
- MinIO-hosted installer files with public download URLs

Endpoints:
Version Management:
- GET / - List all client versions (with platform/active filters)
- GET /{version_id} - Get a version by ID
- POST / - Create a new version (admin only)
- PUT /{version_id} - Update a version (admin only)
- POST /{version_id}/set-latest - Set as latest for platform (admin only)
- DELETE /{version_id} - Delete/deactivate a version (admin only)

Installer Management:
- POST /{version_id}/installer - Upload installer file (admin only)
- GET /{version_id}/installer/download - Download installer (public)

Authentication:
- Read operations: No authentication required
- Write operations: Require admin role
- Download endpoint: Public (no auth) for silent upgrades

Installer Storage:
- Files stored in MinIO S3-compatible storage
- Download URL points to backend endpoint (not direct MinIO)
- Max file size: 50MB
- Allowed formats: .exe, .msi

Version Resolution:
See VersionPolicyService for client-side version policy enforcement.
"""
