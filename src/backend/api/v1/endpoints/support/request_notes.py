...
Provides endpoints for creating and retrieving notes on service requests.
Notes track status changes, hints, and important events throughout the
request lifecycle without requiring chat messages or status updates.

Endpoints:
- GET /{request_id}/notes - Get all notes for a request (paginated)
- GET /notes/{note_id} - Get a specific note by ID
- POST / - Create a new note
- GET /notes/latest - Get latest notes across all requests

Authentication:
- All endpoints require authentication
- GET /notes/latest requires technician role
"""
...
from api.schemas.request_note import (
    RequestNoteCreate,
    RequestNoteDetail,
    RequestNoteRead,
)
