...
Handles screenshot attachments for service requests with automatic
image processing and asynchronous MinIO storage via Celery.

Key Features:
- Automatic compression and resize to max 1920x1080
- Asynchronous MinIO upload via Celery
- Dual storage: temp local (pending) + MinIO (completed)
- Screenshot by filename lookup for chat message integration
- Bulk upload support (up to 50 files)
- Corrupted file detection and handling

Screenshots are stored as Attachment records with storage_type='screenshot'.

Endpoints:
- POST /upload - Upload a screenshot
- GET /{screenshot_id} - Get screenshot metadata by ID
- GET /{screenshot_id}/download - Download screenshot file
- GET /request/{request_id} - Get all screenshots for a request
- GET /by-filename/{filename} - Download screenshot by filename (for chat messages)
- DELETE /{screenshot_id} - Delete a screenshot and its files
- POST /bulk-upload - Upload multiple screenshots

Upload Status Flow:
1. pending - File saved locally, MinIO upload queued
2. completed - File uploaded to MinIO successfully
3. failed - Upload failed, file may be corrupted

Authentication:
- All endpoints require authentication

Storage:
- Temp files: /tmp/screenshots/ (local)
- Permanent: MinIO S3-compatible storage
...
from api.schemas.screenshot import (
    ScreenshotListItem,
    ScreenshotRead,
