...
Provides Active Directory user synchronization and search capabilities.
Maintains a local cache of AD users for fast lookups and auto-complete.

Key Features:
- Paginated user listing with search
- Full-text search across username, email, display_name
- Background AD sync via Celery
- Nuclear replace strategy (delete all, insert all)
- Task status tracking

Endpoints:
- GET / - List domain users with pagination and search
- POST /sync - Trigger AD synchronization (background task)
- GET /sync/status/{task_id} - Get sync task status

Sync Behavior:
The AD sync performs a nuclear replace:
1. Fetches all enabled users from configured AD OUs
2. Deletes all existing domain_users records
3. Bulk inserts new records from AD

This ensures the local cache is always exactly in sync with AD.

Authentication:
- All endpoints require authentication
- Sync endpoint requires technician role

Search Parameters:
- search: Search term (matches username, email, or display_name)
- page: Page number (1-indexed)
- per_page: Items per page (max 100)

Celery Task:
- Task: sync_domain_users_task
- Returns: task_id for status tracking
- Status endpoints: pending, started, success, failure

Note:
Domain users are read-only cache entities. They cannot be created or
modified through this API - only synced from AD.
"""
...
from db import User
from api.schemas.domain_user import (
