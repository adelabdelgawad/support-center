...
class RequestTypeBase(HTTPSchemaModel):
    ...
    name_en: str = Field(
        ..., min_length=2, max_length=100, description="Name in English"
    )
    name_ar: str = Field(
        ..., min_length=2, max_length=100, description="Name in Arabic"
    )
    brief_en: Optional[str] = Field(
        None, max_length=500, description="Brief hint in English"
    )
    brief_ar: Optional[str] = Field(
        None, max_length=500, description="Brief hint in Arabic"
    )
    is_active: bool = True
    section_id: Optional[int] = Field(
        None, description="Service section this request type belongs to"
    )
