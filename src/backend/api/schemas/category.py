...
class CategoryBase(HTTPSchemaModel):
    ...
    name: str = Field(
        ..., min_length=2, max_length=100, description="Internal identifier"
    )
    name_en: str = Field(
        ..., min_length=2, max_length=100, description="Category name in English"
    )
    name_ar: str = Field(
        ..., min_length=2, max_length=100, description="Category name in Arabic"
    )
    description: Optional[str] = Field(None, max_length=500)
    section_id: Optional[int] = Field(None, description="Parent section ID")
    is_active: bool = True
...
class CategoryReadMinimal(HTTPSchemaModel):
    """Minimal category read schema for nested responses."""
...
class SubcategoryBase(HTTPSchemaModel):
    ...
    name_en: str = Field(
        ..., min_length=2, max_length=100, description="Subcategory name in English"
    )
    name_ar: str = Field(
        ..., min_length=2, max_length=100, description="Subcategory name in Arabic"
    )
    description: Optional[str] = Field(None, max_length=500)
    is_active: bool = True
