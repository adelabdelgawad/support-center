...
- Removed deprecated fields (is_escalated, escalation_reason)
- Removed deprecated schemas (ServiceRequestEscalation, ServiceRequestTimeTracking)
- Removed tag_id / TagInfo (tags table dropped in section-based routing refactoring)
"""
...
class ServiceRequestCreateByRequester(HTTPSchemaModel):
    ...
    title: str = Field(
        ..., min_length=5, max_length=200, description="Brief summary of the request"
    )
    request_type_id: Optional[int] = Field(
        None, description="Request type (Incident, Service Request, etc.)"
    )
...
class ServiceRequestUpdateByTechnician(HTTPSchemaModel):
    ...
    description: Optional[str] = Field(
        None, min_length=10, description="Detailed description of the request"
    )
    subcategory_id: Optional[int] = Field(None, description="Request subcategory")
    business_unit_id: Optional[int] = Field(
        None, description="Business unit (can override auto-assigned)"
    )
...
class SubTaskCreate(HTTPSchemaModel):
    ...
    description: Optional[str] = Field(
        None, min_length=10, description="Sub-task description"
    )
    priority_id: Optional[int] = Field(
        3, description="Priority level (defaults to Medium)"
    )
    assigned_to_section_id: int = Field(
        ..., description="Assigned service section (required)"
    )
    assigned_to_technician_id: Optional[UUID] = Field(
        None, description="Assigned technician"
    )
...
class SectionReassignRequest(HTTPSchemaModel):
    """Schema for reassigning a request to a different section."""

    section_id: int = Field(..., description="ID of the section to reassign to")
    reason: Optional[str] = Field(
        None, max_length=500, description="Reason for reassignment"
    )
...
class RequestAssigneesResponse(HTTPSchemaModel):
    ...
    request_id: int  # ID of the service request
